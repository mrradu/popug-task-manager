...
from frontoffice.schemas import Account, AccountUpdateFields, BaseAccount
from frontoffice.service.base import BaseDB
...
class UserDB(BaseDB):
    ...
    def update_user(self, user_fields: AccountUpdateFields):
        print(user_fields.dict(exclude_none=True, exclude={"public_id"}))
        self._db.query(models.AccountsModel).filter(
            models.AccountsModel.public_id == user_fields.public_id
        ).update(user_fields.dict(exclude_none=True, exclude={"public_id"}))
